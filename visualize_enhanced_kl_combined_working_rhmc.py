#!/usr/bin/env python3
"""
Enhanced KL Combined Visualization with Working RHMC from Codebase
================================================================
Creates a combined visualization showing centroid evolution, posterior samples,
and RHMC sampling using the existing working RHMC implementation.
"""
...
def create_combined_visualization_working_rhmc():
    """Create combined visualization with working RHMC from codebase."""
    
    print("🎨 Creating Combined Enhanced KL Visualization with Working RHMC from Codebase")
    print("=" * 60)
    ...
    # Initialize the working RHMC sampler from the codebase
    ...
    rhmc_sampler = RHVAEVolumeElementHMCSampler(
        model=model,
        mcmc_steps_nbr=50,  # Reduced for visualization
        n_lf=10,
        eps_lf=0.02,
        beta_zero=1.0
    )
    ...
    print("\n5️⃣ Running enhanced KL simulation with working RHMC sampling...")
    ...
    for step in range(20):  # 20 steps for cleaner visualization
        ...
        if step % 5 == 0:
            with torch.no_grad():
                ...
                # Sample from working RHMC using the existing sampler
    ...
    print("\n6️⃣ Creating combined visualization...")
    ...
    ax.scatter(rhmc_2d[:, 0], rhmc_2d[:, 1], 
              c='blue', s=12, alpha=0.6, label='Working RHMC Samples (Manifold)')
    ...
    • RHMC Samples: {len(all_rhmc_flat)} (working manifold-following)
    • Final Beta: {metric_evolution[-1]['beta']:.3f}
    • Adaptive KL: Working ✅
    • Metric Updates: Working ✅
    • Working RHMC Sampling: Working ✅
    • RHMC Sampler: RHVAEVolumeElementHMCSampler ✅
    """
    ...
    output_path = "enhanced_kl_combined_working_rhmc_visualization.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"✅ Saved working RHMC combined visualization to: {output_path}")
    
    # Log to WandB
    try:
        wandb.init(project="enhanced-kl-visualization", name="combined-working-rhmc-analysis")
        wandb.log({
            "combined_working_rhmc_visualization": wandb.Image(output_path),
...
if __name__ == "__main__":
    # Create the combined visualization
    results = create_combined_visualization_working_rhmc()
    
    print("\n🎉 Working RHMC Combined Enhanced KL Visualization Complete!")
    print("=" * 60)
    print("📊 Key Insights:")
    ...
    print("   • Working RHMC samples using the existing RHVAEVolumeElementHMCSampler")
    print("   • Manifold determinant background (log scale)")
    print("   • All in a single combined graph!")
    print("   • RHMC sampling now properly works using the existing codebase implementation!")
