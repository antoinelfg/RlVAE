#!/usr/bin/env python3
"""
Download and Process dSprites Dataset (Memory Efficient)
=======================================================

Memory-efficient version that processes the dSprites dataset in chunks
to avoid memory issues on systems with limited RAM.
"""
...
def load_dsprites_data_subset(data_path: Path, max_images: int = 50000) -> dict:
    """Load a subset of the dSprites .npz file to avoid memory issues."""
    print(f"📂 Loading dSprites data subset from {data_path} (max {max_images} images)...")
    
    # Load the data (allow_pickle=True for metadata)
    data = np.load(data_path, allow_pickle=True)
    
    # Extract components (subset only)
    imgs = data['imgs'][:max_images]  # Take only first max_images
    latents_values = data['latents_values'][:max_images]
    latents_classes = data['latents_classes'][:max_images]
    ...
    try:
        ...
        print(f"⚠️ Could not load metadata (encoding issue), using default")
    ...
    print(f"✅ Loaded dSprites dataset subset:")
...
def main():
    ...
    print("🚀 dSprites Dataset Downloader and Processor (Memory Efficient)")
    print("=" * 60)
    ...
    # Load and process data (subset to avoid memory issues)
    max_images = 40000  # Use a subset to avoid memory issues
    data = load_dsprites_data_subset(dsprites_path, max_images)
    ...
    train_metadata = {
        ...
        'source': 'dSprites (DeepMind) - subset',
        'description': 'Real dSprites dataset converted to cyclic RGB sequences (memory-efficient subset)',
        'url': dsprites_url,
        'original_size': max_images
    }
    ...
    backup_dir = processed_dir / "backup_synthetic"
    ...
    for filename in existing_files:
        ...
        if src.exists():
            dst = backup_dir / f"synthetic_{filename}"
    ...
    # Backup existing raw data if it exists
    for raw_path in [raw_train_path, raw_test_path]:
        if raw_path.exists():
            backup_path = backup_dir / f"synthetic_{raw_path.name}"
    ...
    print(f"📂 Synthetic data backed up in: {backup_dir}")
    print(f"   - All previous synthetic files saved with 'synthetic_' prefix")
    
    print("\n✨ Your real dSprites data is now ready!")
    print("You can now run your experiments with the authentic dSprites dataset.")
    print(f"Note: Using subset of {max_images} images for memory efficiency.")
