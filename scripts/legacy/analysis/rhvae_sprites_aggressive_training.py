#!/usr/bin/env python3
"""
RHVAE Aggressive Training on Sprites Data
=========================================

This script trains RHVAE with aggressive hyperparameters to make the metric
really learn the data geometry. Uses the sprites data which has clear
temporal structure and geometric relationships.
"""
...
output_dir = Path("rhvae_sprites_aggressive")
...
def train_aggressive_rhvae():
    """Train RHVAE with aggressive hyperparameters."""
    print("🚀 Training RHVAE with aggressive hyperparameters...")
    
    # Load data
    train_data, eval_data = load_sprites_data()
    
    # AGGRESSIVE Configuration for metric learning
    config = BaseTrainerConfig(
        output_dir='rhvae_sprites_aggressive_model',
    ...
    # AGGRESSIVE RHVAE hyperparameters
    model_config = RHVAEConfig(
        input_dim=(1, 28, 28),  # Fixed: resized sprites data to 28x28
        latent_dim=8,  # Smaller latent dim for easier metric learning
        n_lf=3,  # More leapfrog steps
        eps_lf=0.01,  # Larger step size
        beta_zero=0.1,  # Smaller beta for more aggressive sampling
        temperature=0.5,  # Lower temperature for sharper metric
        regularization=0.0001  # Much lower regularization
    )
    ...
    print("🚀 Starting aggressive RHVAE training...")
    pipeline(
        train_data=train_data,
        eval_data=eval_data
    )
    
    return 'rhvae_sprites_aggressive_model'
...
def load_trained_model():
    ...
    model_dir = "rhvae_sprites_aggressive_model/RHVAE_training_*/final_model"
    import glob
...
def analyze_metric_tensor(model, n_samples=100):
    ...
    axes[0, 0].hist(det_values, bins=20, alpha=0.7)
    ...
    axes[0, 1].hist(log_det_values, bins=20, alpha=0.7)
    ...
    axes[1, 0].hist(all_eigenvals.real, bins=30, alpha=0.7)
    axes[1, 0].set_xlabel('Eigenvalue (Real Part)')
    axes[1, 0].set_ylabel('Count')
    axes[1, 0].set_title('Metric Eigenvalue Distribution')
    
    # Plot 4: Metric determinant vs latent dimension
    latent_dim1 = z_samples[:len(det_values), 0].cpu().numpy()
    axes[1, 1].scatter(latent_dim1, det_values, alpha=0.6)
    ...
    print(f"📊 Metric determinant stats: min={det_values.min():.6f}, max={det_values.max():.6f}, mean={det_values.mean():.6f}, std={det_values.std():.6f}")
...
def main():
    ...
    print("🚀 RHVAE Aggressive Training on Sprites")
    print("=" * 50)
    
    # Train the model
    model_dir = train_aggressive_rhvae()
    ...
    print(f"   3. If still flat, try even more aggressive hyperparameters")
