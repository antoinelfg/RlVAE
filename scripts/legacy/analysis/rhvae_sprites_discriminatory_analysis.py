#!/usr/bin/env python3
"""
RHVAE Sprites Discriminatory Analysis
=====================================

Analyze the discriminatory RHVAE model to show much sharper metric discrimination.
"""
...
def load_trained_model():
    """Load the discriminatory RHVAE model."""
    model_dir = "rhvae_sprites_discriminatory/RHVAE_training_*/final_model"
    import glob
    ...
    trained_model.eval()
    print(f"✅ Loaded discriminatory model from {latest_model}")
...
def compute_discriminatory_metric_at_points(model, points):
    """Compute metric tensor with discriminatory analysis."""
    print("📏 Computing discriminatory metric tensors...")
    metric_determinants = []
    metric_traces = []
...
def create_discriminatory_visualization(model, latent_points, metric_dets, metric_traces, metric_conditions, metric_distances):
    """Create visualization showing discriminatory behavior."""
    print("🎨 Creating discriminatory visualization...")
    ...
    ax2.set_title('Metric Determinant vs Distance\n(Discriminatory Behavior)', fontsize=14, fontweight='bold')
    ax2.set_yscale('log')
    ...
    ax3.set_title('Metric Trace vs Distance\n(Discriminatory Behavior)', fontsize=14, fontweight='bold')
    
    # 4. High-resolution metric determinant heatmap
    ax4 = plt.subplot(2, 3, 4)
    print("  Computing discriminatory metric heatmap...")
    ...
    grid_metric_dets, _, _, _ = compute_discriminatory_metric_at_points(model, full_grid_points)
    ...
    ax4.set_title('Discriminatory Metric Determinant\n(Sharp Variation)', fontsize=14, fontweight='bold')
    ...
    ax5.set_title('Discriminatory Metric Determinant Distribution\n(Sharp Peaks)', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('rhvae_sprites_discriminatory_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("✅ Discriminatory analysis saved to rhvae_sprites_discriminatory_analysis.png")

def create_sharp_heatmaps(model, latent_points):
    """Create heatmaps showing sharp discriminatory behavior."""
    print("🔥 Creating sharp discriminatory heatmaps...")
    ...
    # Compute metric at grid points
    grid_metric_dets, grid_metric_traces, grid_metric_conditions, grid_distances = compute_discriminatory_metric_at_points(model, full_grid_points)
    grid_metric_dets = grid_metric_dets.reshape(xx.shape)
    ...
    # Plot 1: Sharp metric determinant heatmap
    ...
    axes[0, 1].set_title('Sharp Discriminatory Metric Trace\n(150x150 Grid)', fontsize=12, fontweight='bold')
    ...
    axes[1, 1].set_title('Sharp Discriminatory Condition Number\n(150x150 Grid)', fontsize=12, fontweight='bold')
    axes[1, 1].set_xlabel('Latent Dim 1')
    axes[1, 1].set_ylabel('Latent Dim 2')
    plt.colorbar(im4, ax=axes[1, 1], label='Condition Number')
    
    plt.tight_layout()
    plt.savefig('rhvae_sprites_discriminatory_heatmaps.png', dpi=300, bbox_inches='tight')
    plt.close()
    
    print("✅ Sharp discriminatory heatmaps saved to rhvae_sprites_discriminatory_heatmaps.png")

def main():
    """Main discriminatory analysis function."""
    print("🎯 RHVAE Sprites Discriminatory Analysis")
    print("=" * 40)
    ...
    # Compute discriminatory metric at all points
    metric_dets, metric_traces, metric_conditions, metric_distances = compute_discriminatory_metric_at_points(model, latent_points)
    
    # Create discriminatory visualizations
    create_discriminatory_visualization(model, latent_points, metric_dets, metric_traces, metric_conditions, metric_distances)
    create_sharp_heatmaps(model, latent_points)
    
    # Print discriminatory summary statistics
    print("\n📊 Discriminatory Summary Statistics:")
    ...
    # Analyze discriminatory behavior
    print("\n🎯 Discriminatory Behavior Analysis:")
    ...
    print(f"   - Sharpness ratio (far/near determinant): {np.mean(metric_dets[metric_distances > 2.0]) / np.mean(metric_dets[metric_distances < 0.5]):.2f}")
    
    print("\n✅ Discriminatory analysis completed!")
    print("📁 Files created:")
    print("   - rhvae_sprites_discriminatory_analysis.png")
    print("   - rhvae_sprites_discriminatory_heatmaps.png")
