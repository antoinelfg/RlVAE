#!/usr/bin/env python3
"""
Demo Visualizations with Consistent PCA
======================================

This script creates demonstration visualizations for the RLVAE model
with consistent PCA transformation across all plots.
...
def disable_gradients_for_visualization(model):
    """Disable gradients for all model parameters during visualization."""
    for param in model.parameters():
        param.requires_grad_(False)
    return model

def get_consistent_pca_transformation(mus, random_state=42):
    """Get a consistent PCA transformation for all visualizations."""
    # Use a fixed PCA transformation for consistency across visualizations
    # This ensures the same latent space projection for all plots
    pca = PCA(n_components=2, random_state=random_state)
    
    # Fit PCA on posterior means for consistent projection
    mus_2d = pca.fit_transform(mus.numpy())
    
    return pca, mus_2d

def extract_latent_representations(output, model, batch_size):
...
class RealDataLoader:
    ...
    def __init__(self, batch_size=64, num_batches=20, input_dim=[3, 64, 64]):
...
def create_phase1_demo_visualization(model: RiemannianFlowVAE, save_path: Path):
    ...
    dataloader = RealDataLoader(batch_size=64, num_batches=20, input_dim=input_dim)
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            try:
                ...
                batch_mus, batch_samples = extract_latent_representations(output, model, batch.shape[0])
    ...
    # Use consistent PCA transformation for all visualizations
    pca, mus_2d = get_consistent_pca_transformation(mus, random_state=42)
    ...
    ax1.set_title('Phase 1: Latent Space (Metric Frozen) - Consistent PCA')
    ax1.set_xlabel('PC1')
    ax1.set_ylabel('PC2')
    ax1.legend()
...
def create_phase2_demo_visualization(model: RiemannianFlowVAE, save_path: Path):
    ...
    ax1.set_title('Phase 2: Latent Space Evolution - Consistent PCA')
    ax1.set_xlabel('PC1')
    ax1.set_ylabel('PC2')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
...
def main():
    ...
    try:
        ...
        phase1_path = output_dir / "phase1_demonstration_consistent_pca.png"
        ...
        phase2_path = output_dir / "phase2_demonstration_consistent_pca.png"
        ...
        print("   ✅ phase1_demonstration_consistent_pca.png")
        print("   ✅ phase2_demonstration_consistent_pca.png")
        ...
        print("\n🎯 Consistent PCA Transformation:")
        print("   ✅ Same PCA transformation used across all visualizations")
        print("   ✅ Fixed random_state=42 for reproducible projections")
        print("   ✅ Ensures consistent latent space visualization")
