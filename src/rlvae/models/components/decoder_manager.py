...
try:
    from src.models.rhvae_experiment import RGBDecoder as RHVAE_RGBDecoder
except Exception:
    RHVAE_RGBDecoder = None
...
class DecoderManager(nn.Module):
    ...
    def _create_resnet_decoder(self) -> BaseDecoder:
        ...
        class ResNetDecoder(BaseDecoder):
            def __init__(self, input_dim, latent_dim, config):
                ...
                hidden_dims = config.get('hidden_dims', config.get('layers', [512, 256, 128, 64]))
        ...
        # Use the decoder config directly; do not expect a nested 'resnet' key
        return ResNetDecoder(self.input_dim, self.latent_dim, self.config)
    ...
    def _create_rhvae_rgb_decoder(self) -> BaseDecoder:
        """Create the exact RHVAE RGB decoder used in Stage A."""
        if RHVAE_RGBDecoder is None:
            raise RuntimeError("RHVAE_RGBDecoder not available")
        from types import SimpleNamespace
        args = SimpleNamespace()
        args.input_dim = self.input_dim
        args.latent_dim = self.latent_dim
        return RHVAE_RGBDecoder(args)
    ...
    def load_pretrained(self, path: str) -> None:
        ...
        try:
            ...
            if not p.exists() or not p.is_file():
                raise FileNotFoundError(f"Pretrained decoder file not found: {path}")
            ...
            try:
                ...
                print(f"⚠️ Prefix format failed: {e}")
                pass
            ...
            try:
                ...
                if dec_sd:
                    ...
                    print(f"✅ Loaded pretrained decoder from: {path} (extracted from model_state_dict)")
                    return
            except Exception:
                pass
            ...
            if environ.get("RLVAE_STRICT_PRETRAIN", "0") == "1":
                raise RuntimeError(f"Strict decoder load failed for {path}; aborting due to RLVAE_STRICT_PRETRAIN=1")
            ...
            try:
                ...
                print(f"⚠️ Loaded pretrained decoder from: {path} (partial load): missing={len(missing)}, unexpected={len(unexpected)}")
                if missing:
