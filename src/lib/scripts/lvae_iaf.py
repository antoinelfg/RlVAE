...
import torch
import logging
from src.lib.src.pythae.models import VAE_IAF, VAE_IAF_Config, AutoModel
...
def main(args):
    ...
    if args.dataset == "colormnist":
        ...
        train_data = torch.load(os.path.join('../../data/faces/faces_front.pt'), map_location="cpu")
        eval_data = torch.load(os.path.join('../../data/faces/faces_front.pt'), map_location="cpu")
        test_data = torch.load(os.path.join('../../data/faces/faces_front.pt'), map_location="cpu")

    elif args.dataset == "faces_64":
        args.input_dim = (3, 64, 64)
        train_data = torch.load(os.path.join('../../data/faces/faces_front_64x64.pt'), map_location="cpu")
        eval_data = torch.load(os.path.join('../../data/faces/faces_front_64x64.pt'), map_location="cpu")
        test_data = torch.load(os.path.join('../../data/faces/faces_front_64x64.pt'), map_location="cpu")
    ...
    if args.dataset == "colormnist":
        ...
        model = VAE_IAF(config, encoder, decoder).cuda()
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer,
            milestones=[2000, 4000, 6000, 8000, 10000, 12000, 14000, 16000, 18000, 20000, 22000, 24000, 26000, 28000],
            gamma=10**(-1/6),
            verbose=True
        )
