...
class MADE(BaseNF):
    ...
    def __init__(self, model_config: MADEConfig):
        ...
        # Initialize log_var bias in the final MaskedLinear to -2.0
        # The last MaskedLinear outputs 2 * output_dim, so bias shape is (2 * output_dim,)
        with torch.no_grad():
            if hasattr(self.net[-1], 'bias'):
                self.net[-1].bias[self.input_dim:].fill_(-2.0)
    ...
    def forward(self, x: torch.tensor, h=None, **kwargs) -> ModelOutput:
        ...
        # Clamp log_var to prevent flow explosion (tighter)
        max_log_s = 1.5
        log_var = torch.clamp(log_var, -max_log_s, max_log_s)
