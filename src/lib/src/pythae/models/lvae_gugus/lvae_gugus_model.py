...
class LVAE_GUGUS(VAE):
    ...
    def __init__(
        ...
        self.GM = None  # To be set dynamically via retrieveG during training if needed
    ...
    def _log_p_z(self, z):
        if self.prior == "standard":
            log_det_Gz = torch.logdet(self.metric_tensor_field(z))
            log_p_z = -0.5 * log_det_Gz
    ...
    def _compute_recon_loss(self, recon_x, x, pix_mask, sequential=False, seq_mask=None):
        if sequential:
            ...
            recon_loss = (loss * mask).sum(dim=-1).reshape(batch_size, n_obs) * seq_mask
    ...
    def retrieveG(self, data, vi_index=0, verbose=False, T_multiplier=1.0, device='cpu', addStdNorm=True):
        ...
        Returns:
            ...
            G_avg (torch.Tensor): The average metric tensor (if computed).
        """
        self = self.to(device)
        data = data.to(device)
        N = data.shape[0]
        ...
        mu_avg = mu.mean(dim=0, keepdim=True)  # (1, latent_dim)
        self.GM = mu_avg  # for HMC sampling later on

        if addStdNorm:
            std_avg = torch.exp(0.5 * log_var).mean(dim=0, keepdim=True)  # (1, latent_dim)
            G = torch.diag_embed(std_avg.squeeze()**2)  # (latent_dim, latent_dim)
        else:
            G = torch.eye(self.latent_dim).to(device)

        if verbose:
            print(f"Retrieved Riemannian mean (μ) from {N} samples at vi={vi_index}")
            print(f"Mean norm: {mu_avg.norm().item():.4f}")
            print(f"Avg std (per dim): {torch.exp(0.5 * log_var).mean(0)}")
            print(f"Riemannian Metric trace: {G.trace().item():.4f}")
        
        return mu_avg, log_var

    def metric_tensor_field(self, z):
        # Stub: return identity matrix for each z (standard Gaussian prior)
        latent_dim = z.shape[-1]
        batch_size = z.shape[0]
        return torch.eye(latent_dim, device=z.device).unsqueeze(0).repeat(batch_size, 1, 1)