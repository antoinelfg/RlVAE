...
class LVAE_GUGUS(VAE):
    ...
    def __init__(
        ...
        # store a metric tensor for each observation time (t = 0 … n_obs‑1)
        self.G_list: list[Optional[torch.Tensor]] = [None] * self.n_obs
        ...
        # ──────────────────────────────────────────────────────────────────
    @torch.no_grad()
    def retrieveG_all(self, data, verbose: bool = False, device: str = 'cpu', addStdNorm: bool = True):
        """
        Estimates and stores a diagonal metric tensor G_t for **each** observation
        time t = 0 … n_obs‑1.

        After calling this function you can access the list `self.G_list[t]`
        and the mean latent `self.GM_list[t]` for any timestep.

        Args:
            data (torch.Tensor): shape (N, n_obs, …)
            verbose (bool): print basic stats for every t
            device (str): computation device
            addStdNorm (bool): include posterior std in G_t (diag‑cov)

        Returns:
            GM_list (List[torch.Tensor]): mean latent for every t  (n_obs, 1, D)
            G_list  (List[torch.Tensor]): metric tensors G_t        (n_obs, 1, D, D)
        """
        self = self.to(device)
        data = data.to(device)
        ...
        for t in range(self.n_obs):
            ...
            GM_list.append(mu_avg)

            if addStdNorm:
                std_avg = torch.exp(0.5 * log_var).mean(dim=0, keepdim=True)
                G_t = torch.diag_embed(std_avg.squeeze() ** 2)            # (D, D)
            else:
                G_t = torch.eye(self.latent_dim, device=device)

            G_list.append(G_t.unsqueeze(0))                                # (1, D, D)

            # persist
            self.G_list[t] = G_t.unsqueeze(0).to(device)

            if verbose:
                detG = torch.det(G_t).item()
                eig = torch.linalg.eigvalsh(G_t)
                print(f"t={t}  det(G)={detG:.3e}  λmin={eig.min():.2e}  λmax={eig.max():.2e}")

        # keep GM of t=0 for backward compatibility
        self.GM = GM_list[0]
        self.G0 = G_list[0]

        return GM_list, G_list
    ...
    def _log_p_z(self, z, t=0):
        ...
        if self.prior == "standard":
            # Prefer pre‑computed metric if available
            if hasattr(self, "G_list") and self.G_list[t] is not None:
                G_t = self.G_list[t].to(z.device).expand(z.shape[0], -1, -1)
            else:
                G_t = self.metric_tensor_field(z, t)  # fallback push‑forward
    ...
    def metric_tensor_field(self, z, t):
        ...
        If `self.G_list[t]` is available it takes precedence; otherwise the
        method falls back to pushing G0 through the flows.
        Args:
