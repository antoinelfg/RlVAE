...
class LVAE_GUGUS(VAE):
    ...
    @torch.no_grad()
    def retrieveG(
        self, data, vi_index=0, num_centroids=20, knn_neighbors=30, 
        T_multiplier=1.0, device='cuda', verbose=False, addStdNorm=True
    ):
        """
        Estimates local Riemannian metrics using K-Medoids clustering, local covariance around centroids,
        and computes a global temperature based on minimal inter-centroid distances.

        Args:
            data (torch.Tensor): Input data (N, n_obs, ...)
            vi_index (int): Observation index to condition on
            num_centroids (int): Number of clusters (centroids)
            knn_neighbors (int): Number of neighbors for local covariance estimation
            T_multiplier (float): Scaling factor for temperature (local metric)
            device (str): Device for computation
            verbose (bool): Print detailed info
            addStdNorm (bool): Include covariance in metric (if False: identity)

        Returns:
            mu_avg (torch.Tensor): Global mean latent embedding (1, latent_dim)
            G0 (torch.Tensor): Metric tensor from centroid nearest global mean
            T (float): Global temperature computed from minimal centroid distances
        """
        ...
        encoder_output = self.encoder(data[:, vi_index])
        mu, log_var = encoder_output.embedding, encoder_output.log_covariance
        ...
        kmedoids = KMedoids(n_clusters=num_centroids).fit(mu.cpu().numpy())
        centroids = torch.tensor(kmedoids.cluster_centers_, device=device)
        centroids_idx = kmedoids.medoid_indices_

        # Step 3: Compute global temperature T from centroids
        if verbose:
            print("Finding temperature")

        T_is = []
        for i in range(len(centroids)):
            mask = torch.tensor([k for k in range(len(centroids)) if k != i], device=device)
            dists = torch.norm(centroids[i] - centroids[mask], dim=-1)
            T_i = torch.min(dists).item()
            T_is.append(T_i)

        T = np.max(T_is) * T_multiplier

        if verbose: 
            print(f"Best temperature found: {T:.4f}")
            print("Building local metrics")

        # Step 4: kNN around centroids to build local metrics
        knn = NearestNeighbors(n_neighbors=knn_neighbors).fit(mu.cpu().numpy())
        _, neighbors = knn.kneighbors(centroids.cpu().numpy())

        local_metrics = []
        for idx in range(num_centroids):
            neighbor_indices = neighbors[idx]
            neighbor_points = mu[neighbor_indices]

            if addStdNorm:
                local_cov = torch.cov(neighbor_points.T) + 1e-4 * torch.eye(mu.shape[1], device=device)
                G_local = local_cov
            else:
                G_local = torch.eye(mu.shape[1], device=device)

            local_metrics.append(G_local)

        # Step 5: Select centroid closest to global mean as representative
        dists_to_global_mean = torch.norm(centroids - mu_avg, dim=1)
        nearest_centroid_idx = torch.argmin(dists_to_global_mean)
        G0 = local_metrics[nearest_centroid_idx]

        # Step 6: Call build_metrics (your existing function)
        self.build_metrics(
            mu, log_var, centroids_idx,
            addStdNorm=addStdNorm, T=T, lbd=0.01, verbose=verbose
        )
        self.centroids_tens = mu

        # Store G0
        self.G0 = G0.unsqueeze(0)

        if verbose:
            det_G0 = torch.det(G0)
            print(f"Selected centroid index: {nearest_centroid_idx}, det(G0)={det_G0:.3e}")
