...
class LVAE_GUGUS(VAE):
    ...
    def _push_metric(self, z: torch.Tensor, G: torch.Tensor, flow):
        """
        Push-forward G through one flow *with gradient tracking*.
        Computes a per-sample Jacobian (latent_dim * latent_dim), so it is invertible.
        """
        device = z.device
        G = G.to(device)
        ...
        def f_single(y):
            # Flow expects a batch; add/strip batch dim
            out = flow(y.unsqueeze(0))
            return out.out.squeeze(0) if hasattr(out, "out") else out.squeeze(0)
        ...
        for i in range(batch_size):
            zi = z[i]
            zi_next = f_single(zi)
            z_next_list.append(zi_next)

            Ji = torch.autograd.functional.jacobian(
                f_single, zi, create_graph=True
            )                           # (latent_dim, latent_dim)
            J_list.append(Ji)

        z_next = torch.stack(z_next_list, dim=0)     # (B, D)
        J = torch.stack(J_list, dim=0)               # (B, D, D)
