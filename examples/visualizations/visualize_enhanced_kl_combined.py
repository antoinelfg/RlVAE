#!/usr/bin/env python3
"""
Enhanced KL Combined Visualization with RHMC Sampling
===================================================
Creates a combined visualization showing centroid evolution, posterior samples,
and RHMC sampling on the manifold with determinant background.
"""
...
def create_combined_visualization():
    """Create combined visualization with centroid evolution, posterior samples, and RHMC sampling."""
    
    print("🎨 Creating Combined Enhanced KL Visualization with RHMC")
    print("=" * 60)
    ...
    print("\n4️⃣ Running enhanced KL simulation with RHMC sampling...")
    
    # Simulate training steps with metric updates
    for step in range(30):  # 30 steps for cleaner visualization
        ...
        if step % 5 == 0:
            with torch.no_grad():
                ...
                # Sample from RHMC on manifold (timestep 0)
                print(f"   Sampling RHMC for step {step}...")
                rhmc_samples = sample_rhmc_on_manifold(model, n_samples=100, device=device)
    ...
    for i, (centroids_step, color) in enumerate(zip(centroids_2d, colors)):
        ...
        ax.scatter(centroids_step[:, 0], centroids_step[:, 1], 
                  ...
                  label=f'Centroids Step {step_num}' if i % 3 == 0 else "")
    
    # Plot posterior samples
    ax.scatter(posterior_2d[:, 0], posterior_2d[:, 1], 
              c='red', s=10, alpha=0.6, label='Posterior Samples')
    
    # Plot RHMC samples
    ax.scatter(rhmc_2d[:, 0], rhmc_2d[:, 1], 
              c='blue', s=8, alpha=0.5, label='RHMC Samples')
    
    # Add legend and labels
    ax.set_xlabel('PCA Component 1')
    ax.set_ylabel('PCA Component 2')
    ax.set_title('Combined Visualization: Centroids Evolution, Posterior Samples, and RHMC Sampling\nwith Manifold Determinant Background', 
    ...
    • Posterior Samples: {len(all_posterior_flat)}
    • RHMC Samples: {len(all_rhmc_flat)}
    ...
    output_path = "enhanced_kl_combined_visualization.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"✅ Saved combined visualization to: {output_path}")
    
    # Log to WandB
    try:
        wandb.init(project="enhanced-kl-visualization", name="combined-rhmc-analysis")
        wandb.log({
            "combined_visualization": wandb.Image(output_path),
...
def sample_rhmc_on_manifold(model, n_samples=100, device='cuda'):
    """Sample from RHMC on the manifold for timestep 0."""
    
    print(f"   🎯 Sampling {n_samples} RHMC samples on manifold...")
    ...
    for step in range(n_steps):
        ...
        try:
            ...
            # Compute gradient
            grad = torch.autograd.grad(log_prob.sum(), z)[0]
            
            # Leapfrog integration
            p = torch.randn_like(z)  # Momentum
...
if __name__ == "__main__":
    # Create the combined visualization
    results = create_combined_visualization()
    
    print("\n🎉 Combined Enhanced KL Visualization Complete!")
    print("=" * 60)
    print("📊 Key Insights:")
