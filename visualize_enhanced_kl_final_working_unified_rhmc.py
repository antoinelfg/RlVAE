#!/usr/bin/env python3
"""
Enhanced KL Visualization - FINAL WORKING VERSION (UNIFIED RHMC SAMPLING)
=======================================================================

This script uses the SAME full RHMC sampling method for both prior and posterior samples
to ensure they both properly follow the manifold structure like in real RHVAE experiments.

Key Features:
- Uses RHVAEVolumeElementHMCSampler with PROPER parameters from three-stage pipeline
- UNIFIED SAMPLING: Both prior and posterior use the same full RHMC method
...
- FIXED: Unified sampling method for consistent manifold following
"""
...
def sample_unified_rhmc_prior(model, device, n_samples=200):
    """Sample from prior using full RHMC with proper parameters."""
    print(f"\n🎯 Sampling {n_samples} prior samples using UNIFIED full RHMC...")
    ...
    print(f"   📊 Using UNIFIED RHMC parameters:")
    print(f"      - mcmc_steps_nbr: 200 (full exploration)")
    ...
    rhmc_sampler = RHVAEVolumeElementHMCSampler(
        model=model,
        mcmc_steps_nbr=200,  # Full exploration
    ...
    print(f"   ✅ Prior RHMC sampling completed")
...
def sample_unified_rhmc_posterior(model, device, n_samples=128):
    """Sample posterior using the SAME full RHMC method as prior."""
    print(f"\n🧠 Sampling {n_samples} posterior samples using UNIFIED full RHMC...")
    
    # Use the EXACT SAME parameters as prior sampling
    step_size = 0.001
    tuned_eps = min(0.005, max(1e-5, step_size))
    
    print(f"   📊 Using SAME UNIFIED RHMC parameters:")
    print(f"      - mcmc_steps_nbr: 200 (same as prior)")
    print(f"      - n_lf: 30 (same as prior)")
    print(f"      - eps_lf: {tuned_eps:.6f} (same as prior)")
    print(f"      - beta_zero: 1.0 (same as prior)")
    
    # Create RHMC sampler with SAME parameters as prior
    rhmc_sampler = RHVAEVolumeElementHMCSampler(
        model=model,
        mcmc_steps_nbr=200,  # SAME as prior
        n_lf=30,            # SAME as prior
        eps_lf=tuned_eps,   # SAME as prior
        beta_zero=1.0,      # SAME as prior
    )
    
    # Use the SAME full sampling method as prior
    # This ensures both prior and posterior samples follow the manifold structure
    posterior_samples = rhmc_sampler.sample(n_samples)
    
    print(f"   ✅ Posterior RHMC sampling completed")
    print(f"   ✅ Acceptance rate: {rhmc_sampler.last_acceptance_rate:.3f}")
...
def create_unified_visualization(model, device, metric_evolution, prior_samples, posterior_samples):
    """Create the final unified visualization with SAME RHMC method for both."""
    print(f"\n🎨 Creating unified visualization with SAME RHMC method for both prior and posterior...")
    ...
    # Plot posterior samples (UNIFIED RHMC) - SAME PCA projection
    ax.scatter(posterior_pca[:, 0], posterior_pca[:, 1], 
              c='blue', s=20, alpha=0.7, label='Posterior Samples (UNIFIED RHMC)', edgecolors='none')
    
    # Plot prior samples (UNIFIED RHMC) - SAME PCA projection
    ax.scatter(prior_pca[:, 0], prior_pca[:, 1], 
              c='red', s=30, alpha=0.8, label='Prior Samples (UNIFIED RHMC)', edgecolors='none')
    ...
    ax.set_title('Enhanced KL Visualization: UNIFIED RHMC Sampling (Both Prior & Posterior Follow Manifold)', fontsize=14, fontweight='bold')
...
Prior Samples: {len(prior_samples)} (UNIFIED RHMC)
Posterior Samples: {len(posterior_samples)} (UNIFIED RHMC)
...
UNIFIED Parameters: mcmc_steps=200, n_lf=30, eps_lf=0.001 (SAME for both)
FIXED: Same PCA projection for manifold and samples
FIXED: Same sampling method ensures both follow manifold structure"""
    ...
    output_file = 'enhanced_kl_final_working_unified_rhmc_visualization.png'
...
def main():
    """Main function to run the unified enhanced KL visualization."""
    print("🚀 Enhanced KL Visualization - FINAL WORKING VERSION (UNIFIED RHMC SAMPLING)")
    print("=" * 80)
    ...
    # Sample using UNIFIED RHMC method for both prior and posterior
    prior_samples = sample_unified_rhmc_prior(model, device, n_samples=200)
    posterior_samples = sample_unified_rhmc_posterior(model, device, n_samples=128)
    
    # Create unified visualization
    output_file = create_unified_visualization(
        model, device, metric_evolution, prior_samples, posterior_samples
    )
    
    print(f"\n🎉 SUCCESS: Enhanced KL visualization with UNIFIED RHMC sampling completed!")
    print(f"📁 Output file: {output_file}")
    print(f"📊 Key features:")
    print(f"   ✅ Uses proven RHVAEVolumeElementHMCSampler from three-stage pipeline")
    print(f"   ✅ UNIFIED SAMPLING: Both prior and posterior use SAME full RHMC method")
    ...
    print(f"   ✅ FIXED: Same sampling method ensures both follow manifold structure")
